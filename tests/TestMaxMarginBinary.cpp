...
BOOST_AUTO_TEST_CASE(testMaxMarginBinary)
{
  ...
  MaxMarginBinary objective(trainData, models);
  ...
  BOOST_CHECK_CLOSE(0.65, gradFv.getValueAtLocation(1).value(), 1e-8);
  BOOST_CHECK_CLOSE(0.30, gradFv.getValueAtLocation(2).value(), 1e-8);
  BOOST_CHECK_CLOSE(2.35, gradFv.getValueAtLocation(3).value(), 1e-8);
